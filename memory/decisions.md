...
- **2026-06-11 — §12 software quality-gap cleanup (4-phase pass)**
  ...
  - Impact: 14 data files. Zero software-type entries left without license; zero §12 entries without tags. Validation passes (217 warnings, all pre-existing freeform-tech-tag notices like `paper`/`pipeline`). README untouched. Open follow-up: `Massive Software` (misc) duplicates `Massive` (pipeline) — left both, flagged for future dedup.
